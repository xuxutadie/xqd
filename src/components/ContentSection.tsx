...
export default function ContentSection({ title, type }: ContentSectionProps) {
  ...
  function VideoThumbnail({ src, alt }: { src: string; alt?: string }) {
    ...
    return thumb ? (
      <img src={thumb} alt={alt || '视频缩略图'} className="w-full h-full object-cover" />
    ) : (
      <div className="w-full h-full bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center">
        <span className="text-gray-400 text-sm">生成视频缩略图中…</span>
      </div>
    )
  }
  ...
  return (
    <div className="mb-12">
      <div className="flex justify-between items-center mb-6">
        ...
        <a href={`/${type}`} className="text-blue-600 hover:underline">查看更多 {getTypeName()} </a>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {data.map((item, index) => (
            <a
              ...
              className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 border border-gray-100 transform hover:-translate-y-1 block"
              aria-label={`查看${getTypeName()}：${item.title || item.name}`}
            >
              <div className="h-48 bg-gray-50 relative overflow-hidden">
                {item.imageUrl ? (
                  <img src={item.imageUrl} alt={item.title || item.name} className="w-full h-full object-cover transition-transform duration-500 hover:scale-110" />
                ) : type === 'courses' && item.videoUrl ? (
                  <VideoThumbnail src={item.videoUrl} alt={item.title || item.name} />
                ) : (
                  <div className="w-full h-full bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center">
                ...
                {item.badge && (
                  <span className="absolute top-3 left-3 bg-blue-100 text-blue-600 text-xs font-medium px-2.5 py-0.5 rounded-full">{item.badge}</span>
                )}
              </div>
              <div className="p-5 space-y-2 min-h-[120px]">
                <h3 className="text-lg font-semibold text-gray-900 mb-1 leading-tight line-clamp-1">{item.title || item.name}</h3>
                {item.authorName && (
                  <div className="flex items-center space-x-1">
                    <span className="text-gray-600 text-xs">作者:</span>
                    <span className="text-gray-800 text-sm font-medium">{item.authorName}</span>
                  </div>
                )}
                {item.className && (
                  <div className="flex items-center space-x-1">
                    <span className="text-gray-600 text-xs">年级:</span>
                    <span className="text-gray-800 text-sm">{item.className}</span>
                  </div>
                )}
                {item.studentName && (
                  <div className="flex items-center space-x-1">
                    <span className="text-gray-600 text-xs">获奖人:</span>
                    <span className="text-gray-800 text-sm font-medium">{item.studentName}</span>
                    <span className="text-gray-400 text-xs">· 年级:</span>
