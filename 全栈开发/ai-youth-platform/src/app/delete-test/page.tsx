...
export default function DeleteTestPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-4">删除功能测试</h1>
      
      <div className="mb-4">
        <p>当前用户: {user.username} ({user.role})</p>
        <p>Token状态: {token ? '已获取' : '未获取'}</p>
        <p>Token前缀: {token ? token.substring(0, 20) + '...' : '无'}</p>
      </div>
      ...
      <div className="space-y-4">
        <div>
          ...
          <button
            onClick={() => testDelete('/api/works', 'nonexistent-id')}
            disabled={isLoading}
            className="bg-blue-500 text-white px-4 py-2 rounded"
          >
            {isLoading ? '测试中...' : '测试删除不存在的作品'}
          </button>
        </div>
        ...
        <div>
          ...
          <button
            onClick={() => testDelete('/api/competitions', 'nonexistent-id')}
            disabled={isLoading}
            className="bg-blue-500 text-white px-4 py-2 rounded"
          >
            {isLoading ? '测试中...' : '测试删除不存在的赛事'}
          </button>
        </div>
        ...
        <div>
          ...
          <button
            onClick={() => testDelete('/api/honors', 'nonexistent-id')}
            disabled={isLoading}
            className="bg-blue-500 text-white px-4 py-2 rounded"
          >
            {isLoading ? '测试中...' : '测试删除不存在的荣誉'}
          </button>
        </div>
        ...
        <div>
          ...
          <button
            onClick={() => testDelete('/api/courses', 'nonexistent-id')}
            disabled={isLoading}
            className="bg-blue-500 text-white px-4 py-2 rounded"
          >
            {isLoading ? '测试中...' : '测试删除不存在的课程'}
          </button>
        </div>
      </div>
